...
The deterministic suite covers the error-hold lifecycle end to end: a run ending in an error or context overflow pauses the queue and a bare `agent_settled` flushes nothing into the failed session; the first healthy assistant tail — produced by built-in retry or an external loop such as pi-retry re-prompting from the idle signal — releases the hold and dispatches exactly once; a concluded overflow compaction closes recovery without a post-run and still releases; a `session_compact_failed` recovery, an aborted retry tail, and a merely threshold-triggered compaction after the error all keep the rows parked until an explicit empty-composer `Enter` — only overflow-recovery compaction cycles close the hold without a healthy tail. The real-session integration suite replays the overflow-compaction and built-in-retry recoveries against a live `AgentSession` with the extension loaded.
...
- successful idle dispatch, active-run batch delivery, and draining supersede an older snapshot immediately; the last row writes a tombstone, while a partial synchronous batch failure checkpoints only the unsent tail.

Local verification after the Factory control change: `npm run ci` passes with 114 automated tests.

### Boundary

Persistence rides Pi's public `pi.appendEntry()` / `getEntries()` contract. Snapshot entries are append-only: each committed save, accepted delivery, consumed command row, or clean shutdown supersedes the previous snapshot, the restore path trusts only the newest readable one on the active branch, and superseded lines stay in the session JSONL without ever entering the transcript or model context. Two Pi-flush limits bound what resume can promise: sessions without an assistant response are not written to disk at all, and a hard kill skips `session_shutdown`, so newly queued rows that have not reached another checkpoint can still be lost. Already-checkpointed edits and accepted deliveries remain authoritative. A real-TUI evidence run for the resume flow should still be recorded before release.
